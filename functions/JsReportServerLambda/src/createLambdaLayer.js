...
async function pckg() {
  ...
  const extensionPaths = fileUtils.walkSync(
    'node_modules',
    'jsreport.config.js'
  )
  const relativeExtensionPaths = extensionPaths.map((p) =>
    path.relative(__dirname, p)
  )
  await FS.writeFile(
    'node_modules/locations.json',
    JSON.stringify({
...
async function cleanup() {
  ...
  return Promise.all(
    patterns.map((p) => pLimit(() => rimraf(p, { glob: true })))
  )
}
